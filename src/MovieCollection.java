...
public class MovieCollection
{
  ...
  private void searchCast()
  {
    ...
    ArrayList<Movie> results = new ArrayList<Movie>();
      ...
      for (int i = 0 ; i < movies.size(); i++)
      {
      String actors = movies.get(i).getKeywords();
      String[] actorList = actors.split("\\|");

        for (int x = 0; x < actorList.length; x++)
        {
          String lower = actorList[x].toLowerCase();
          if (lower.contains(searchTerm) && !(finalList.contains(actorList[x])) )
          {
            finalList.add(actorList[x]);
          }
        }
      }
      ...
      {
        int minIndex = j;
        for (int k = j + 1; k < finalList.size(); j++)
        {
          if (finalList.get(k).compareTo(finalList.get(minIndex)) < 0)
          {
            minIndex = k;
          }
        }
        String temp = finalList.get(j);
        finalList.set(j,finalList.get(minIndex));
